...
class SVM:
    ...
    def fit(self, X, y, save_frames=False):
        n_samples, n_features = X.shape
        y_ = np.where(y <= 0, -1, 1)
        self.w = np.zeros(n_features)
        self.b = 0

        frames = []  # store filenames if save_frames=True
        ...
        for iteration in range(self.n_iters):
            ...
            if save_frames and iteration % 10 == 0:
                filename = f"frame_{iteration}.png"
                self._save_frame(X, y, filename)
                frames.append(filename)

        if save_frames:
            # create GIF
            with imageio.get_writer('svm_training.gif', mode='I', duration=0.5) as writer:
                for filename in frames:
                    image = imageio.imread(filename)
                    writer.append_data(image)
            print("GIF saved as svm_training.gif")

    def predict(self, X):
        approx = np.dot(X, self.w) - self.b
        return np.sign(approx)

    def _save_frame(self, X, y, filename):
